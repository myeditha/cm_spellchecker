...
#                              MLEProbDist, RandomProbDist,WittenBellProbDist)
from prob_h import (FreqDist, ConditionalFreqDist,
                             ConditionalProbDist, DictionaryProbDist,
                             DictionaryConditionalProbDist,
                             LidstoneProbDist, MutableProbDist,
                             MLEProbDist,LaplaceProbDist, RandomProbDist,WittenBellProbDist)
    ...
    def _train(cls, labeled_sequence, test_sequence=None,
        ...
        if estimator is None:
            def estimator(fd, bins):
                return LidstoneProbDist(fd, 10, bins)
...
#	print("V[0] is ", V[0])
#	print("P is ", P)
#	print("O[:, S[unlabeled_sequence[0]]]", O[:, S[unlabeled_sequence[0]]])
        for t in range(1, T):
            for j in range(N):
                vs = V[t-1, :] + X[:, j]
                best = np.argmax(vs)
#		vs_ret.append(vs[best])
                V[t, j] = vs[best] + O[j, S[unlabeled_sequence[t]]]
#		print(vs[best], self._states[j],unlabeled_sequence[t], S[unlabeled_sequence[t]], O[j, S[unlabeled_sequence[t]]])
                B[t, j] = best
	
        current = np.argmax(V[T-1,:])
        sequence = [current]
	vs_ret.append(V[T-1,current])
        for t in range(T-1, 0, -1):
            last = B[t, current]
            sequence.append(last)
	    vs_ret.append(V[t-1,last])
            current = last
	vs_ret.reverse()
        sequence.reverse()
    ...
    def point_prob(self, unlabeled_sequence):
        """
        ...
        T = len(unlabeled_sequence)
        N = len(self._states)
#	print("THis is the unllabeled seq", unlabeled_sequence)
        alpha = self._forward_probability(unlabeled_sequence)
        beta = self._backward_probability(unlabeled_sequence)
        normalisation = logsumexp2(alpha[T-1])
	har_states = [tup[1] for tup in unlabeled_sequence]
        entropies = np.zeros(T, np.float64)
        probs = np.zeros(N, np.float64)
	p_ret = []
	sequence = [0,1,2,3,4,5,6,7,8]
	a = list(map(self._states.__getitem__, sequence))
	har_words = defaultdict(int)
        for t in range(T):
	    har_word = unlabeled_sequence[t][0]
	    har_words[har_word] = defaultdict(int)
	    print("THis is from HMM", unlabeled_sequence[t][0])
            for s in range(N):
                probs[s] = alpha[t, s] + beta[t, s] - normalisation
		print(a,s, har_words, har_word)
		print (a[s])
		print (har_words[har_word][a[s]])
		print (2**probs[s])
		har_words[har_word][a[s]] = 2**probs[s]
    ...
    def point_entropy(self, unlabeled_sequence):
       ...
       # print("THis is the unllabeled seq", unlabeled_sequence)
	    ...
	    for j,st in enumerate(self._states):
		if har_states[t] == st:
			i = j
			break	
		
            p_ret.append(2**probs[i])
        return entropies
...
def demo_pos():
    ...
    hmm = trainer.train_supervised(labelled_sequences[10:],
                    estimator=lambda fd, bins: LidstoneProbDist(fd, 10, bins))
